...
# The base framework of the API -> Hono
...
A route shoud be part of a group of related endpoints. Each group should have its own index file in the routes/ folder. For example, all user-related endpoints should be in routes/user.index.ts.
If the group of routes is big enough, you can create a folder for it inside routes/, and split the endpoints into multiple files. For example, routes/user/create.ts, routes/user/delete.ts, etc.
Each route file should export a Hono Router instance created via createRouter(), which will be imported and registered in src/index.ts via publicRoutes or privateRoutes list in that file.
The "route_group".index.ts of the route group should be the combination of "route_group".routes.ts with "route_group".handlers.ts. Basically, the routes file should only contain the route definitions and the handlers file should contain the actual implementation of the handlers and in "route_group".index.ts you should import both and combine them to export a single Router instance.

## DB

The database schema should be defined using Drizzle ORM in the db/ folder. Each table should have its own file, and related tables can be grouped into subfolders if necessary. Use as much of Drizzle's features as possible to ensure type safety and ease of use.
